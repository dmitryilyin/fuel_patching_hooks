...
eos
  ...
  it 'can get resources section of CIB XML' do
    expect(@class.resources_cib.to_s).to start_with '<resources>'
    expect(@class.resources_cib.to_s).to end_with '</resources>'
  end
  ...
  it 'can determine the number of controller nodes' do
    ...
    expect(@class.cluster_nodes_count).to eq(1)
    @class.stubs(:read_astute_yaml).returns fuel_settings_multi
    @class.fuel_settings_with_renew
    expect(@class.cluster_nodes_count).to eq(2)
  end
  ...
  context 'on a single controller HA cluster' do
    ...
    it 'stops some resources' do
      @class.expects(:stop_resource).with 'heat-engine'
      @class.fuel_settings_with_renew
      @class.stop_or_ban_by_regexp resources_regexp
    end

    it 'starts some resources' do
      @class.expects(:start_resource).with 'heat-engine'
      @class.fuel_settings_with_renew
  ...
  context 'on a multi controller HA cluster' do
    ...
    it 'bans some resources' do
      @class.expects(:ban_resource).with 'heat-engine'
      @class.fuel_settings_with_renew
      @class.stop_or_ban_by_regexp resources_regexp
    end

    it 'unbans some resources' do
      @class.expects(:unban_resource).with 'heat-engine'
      @class.fuel_settings_with_renew
      @class.start_or_unban_by_regexp resources_regexp
    end
  end
